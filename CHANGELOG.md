...
- **Native uninstaller.** `bash uninstall.sh` or `oncellclaw.sh --uninstall` removes the service, data directory, host registration, and OneCLI agent registration for that installation. Dry-run and confirmation modes are included.
...
- [BREAKING] **Install flow replaced.** `bash oncellclaw.sh` is the new default: a scripted installer that hands off to Claude Code for error recovery and guided decisions. The `/setup` Claude-guided skill still works as an alternative.
