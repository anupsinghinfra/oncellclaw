...
- **Agent containers now start with safer defaults.** New spawns drop Linux capabilities, set `no-new-privileges`, and apply a PID limit. Per-group overrides remain available for workloads that deliberately need a capability back. The Vercel CLI is now opt-in instead of being baked into every image.
