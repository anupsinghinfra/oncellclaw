...
1. **Check for existing work.** Search open PRs and issues before starting:
   ```bash
   gh pr list --repo anupsinghinfra/oncellclaw --search "<your feature>"
   gh issue list --repo anupsinghinfra/oncellclaw --search "<your feature>"
   ```
...
Breaking changes are allowed; **silent** ones are not. oncellclaw does not migrate user installs at runtime — the user's coding agent is the migrator, so every breaking change must ship a migration path that agent can execute without a human reverse-engineering the diff:
...
oncellclaw uses [Claude Code skills](https://code.claude.com/docs/en/skills) — markdown files with optional supporting files that teach Claude how to do something. There are four types of skills in oncellclaw, each serving a different purpose.
...
1. Fork `anupsinghinfra/oncellclaw` and branch from `main`
...
Skills that run inside the agent container, not on the host. These teach the oncellclaw agent how to use tools, format output, or perform tasks. They are synced into each group's `.claude/skills/` directory when a container starts.
...
**Key difference:** You never invoke these from a coding-agent session on the host, the way you run `/setup` or `/update-nanoclaw` in Claude Code/Codex/OpenCode. They're mounted into the sandbox and loaded by the oncellclaw agent itself, shaping how it behaves when you chat with it.
...
Agent templates (reusable bundles of instructions + MCP servers + skills) ship in the separate [`anupsinghinfra/oncellclaw-templates`](https://github.com/anupsinghinfra/oncellclaw-templates) repo, not this one. Contribute them there via PR (its README has the anatomy and checklist). For how templates load and the OneCLI credential model, see [docs/templates.md](docs/templates.md).
